...
class NeighborManager:
    ...
    def _build_all_neighbors(self) -> None:
        ...
        # Store general neighbors (for all atoms)
        ...
        # Build oxygen neighbor list
        self._build_oxygen_neighbors(i_list, j_list, d_list, n_atoms)
    ...
    def _log_general_neighbors(self) -> None:
        ...
        O_indices = np.where((self.structure.atom_types == AtomType.OXYGEN) |
                            (self.structure.atom_types == AtomType.OXYGEN_VACANCY))[0]
        ...
        n_O = sum(1 for i in O_indices if self.neighbors_csr.has_neighbors(i))
        ...
        avg_O = np.mean([self.neighbors_csr.n_neighbors(i) for i in O_indices
                        if self.neighbors_csr.has_neighbors(i)]) if n_O > 0 else 0

        logger.info(f"General neighbor list for energy calculations (all atoms within {self.params.cutoff:.1f} Å):")
        logger.info(f"  A-sites: {n_A} atoms (avg {avg_A:.1f} neighbors of any type)")
        logger.info(f"  B-sites: {n_B} atoms (avg {avg_B:.1f} neighbors of any type)")
        logger.info(f"  O-sites: {n_O} atoms (avg {avg_O:.1f} neighbors of any type)")
    ...
    def _build_nearest_neighbors(self, i_list, j_list, d_list, n_atoms) -> None:
        """Build nearest-neighbor lists for cation and oxygen diffusion"""
        nn_distance_A = self.params.nn_distance_A
        nn_distance_B = self.params.nn_distance_B
        nn_distance_O = self.params.nn_distance_O

        logger.info(f"NN cutoffs: A-A = {nn_distance_A:.3f} Å, B-B = {nn_distance_B:.3f} Å, O-O = {nn_distance_O:.3f} Å")
        ...
        for i, j, dist in zip(i_list, j_list, d_list):
            ...
            # A-site connections
            if self.structure.A_mask[i] and self.structure.AB_mask[j] and dist <= nn_distance_A:
                nearest_neighbors_dict[i].append((j, dist))
            # B-site connections
            elif self.structure.B_mask[i] and self.structure.AB_mask[j] and dist <= nn_distance_B:
                nearest_neighbors_dict[i].append((j, dist))
    ...
    def _log_nearest_neighbors(self) -> None:
        ...
        logger.info(f"  A-sites: {n_A} atoms (avg {avg_A:.1f} neighbors within {nn_distance_A:.2f} Å)")
        logger.info(f"  B-sites: {n_B} atoms (avg {avg_B:.1f} neighbors within {nn_distance_B:.2f} Å)")
        logger.info(f"  Memory usage: {self.nearest_neighbors_csr.memory_usage() / 1024:.1f} KB")

    def _build_oxygen_neighbors(self, i_list, j_list, d_list, n_atoms) -> None:
        """Build nearest-neighbor list for oxygen diffusion

        Connect atom i to atom j IF both are oxygen species (O or XO) AND distance < nn_distance_O
        """
        nn_distance_O = self.params.nn_distance_O

        # Filter for nearest oxygen-oxygen connections
        ...
        for i, j, dist in zip(i_list, j_list, d_list):
            ...
            # Oxygen-oxygen connections (including oxygen vacancies)
            if self.structure.O_mask[i] and self.structure.O_mask[j] and dist <= nn_distance_O:
    ...
    def _log_oxygen_neighbors(self) -> None:
        ...
        n_oxygen = sum(1 for i in oxygen_indices if self.oxygen_neighbors_csr.has_neighbors(i))

        avg_oxygen = np.mean([self.oxygen_neighbors_csr.n_neighbors(i) for i in oxygen_indices
                             if self.oxygen_neighbors_csr.has_neighbors(i)]) if n_oxygen > 0 else 0

        nn_distance_O = self.params.nn_distance_O
        logger.info(f"Nearest neighbors for oxygen diffusion (oxygen-oxygen only):")
        logger.info(f"  O-sites: {n_oxygen} atoms (avg {avg_oxygen:.1f} neighbors within {nn_distance_O:.2f} Å)")
